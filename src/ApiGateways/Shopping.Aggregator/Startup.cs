...
namespace Shopping.Aggregator
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                config.BaseAddress = new Uri(Configuration.GetValue<string>("ApiSettings:CatalogUrl"));
            });

            services.AddHttpClient<IBasketService, BasketService>(config =>
            {
                config.BaseAddress = new Uri(Configuration.GetValue<string>("ApiSettings:BasketUrl"));
            });
